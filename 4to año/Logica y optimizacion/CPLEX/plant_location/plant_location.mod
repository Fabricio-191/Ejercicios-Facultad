...
subject to {
	forall(l in Locations) open[l] == (load[l] > 0);
	pack(
		all(l in Locations) load[l],
...
tuple SolutionLocation {
	int load;
	int open;
}
tuple SolutionCustomer {
	int cust;
	int location;
}
tuple SolutionValues {
	int obj;
	float occupancy;
	float minOccup;
}

{SolutionCustomer} solutionCustomer = {<i, cust[i]> | i in Customers};
{SolutionLocation} solutionLocation = {<load[i], open[i]> | i in Locations};
{SolutionValues} solutionValues = {<obj, occupancy, minOccup>};
