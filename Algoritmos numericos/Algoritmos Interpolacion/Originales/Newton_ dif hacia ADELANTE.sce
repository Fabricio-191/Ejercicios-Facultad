function difad()
  ...
  cant = input('Ingrese la cantidad de datos:')
  x0 = input('Ingrese x0: ');
  h = input('Ingrese separación entre muestras: ');
  for i=1:cant  
    t(i,1) = x0 + h*(i-1);
    printf('\n Ingrese valor de ordenada para x = %f: ', t(i,1));
    t(i,2) = input('')
   end
...
disp('Imagen de la absisa x ingresada por teclado:')
  ...
  printf('\n g(%f) = %10.7f', x, gx);
 else printf ('Error en el valor ingresado');
 end 
endfunction
